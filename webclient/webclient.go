...
func setUpEndpoints(router *mux.Router) {
	...
	log.Println("GET /backup/{UUID}")
	router.HandleFunc("/backup/{UUID}", backup.HandlePolling).Methods("GET")
	...
	log.Println("GET /restire/{UUID}")
	router.HandleFunc("/restore/{UUID}", restore.HandlePolling).Methods("GET")
